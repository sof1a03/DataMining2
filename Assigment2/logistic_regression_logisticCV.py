...
def preprocess_text(text):
    ...
    doc = nlp(" ".join(words))
    lemmatized_text = " ".join([token.lemma_ for token in doc if not token.is_punct]) # and token.lemma_ not in stop_wordsand token.lemma_ not in hotel_names])
...
# Logistic Regression with cross-validated C parameter
logistic_cv = LogisticRegressionCV(
    Cs=[0.01, 0.1, 1, 5, 10],         # List of C values to test
    cv=5,                               # 5-fold cross-validation
    scoring='accuracy',                  # Use accuracy score for model selection (or another metric)
    penalty='l1',
    solver='liblinear'
)
...
print("Best C:", logistic_cv.C_[0])
print("Cross-Validation Scores:", logistic_cv.scores_[1].mean(axis=0))
print("Test Set Score:", logistic_cv.score(X_test, y_test))

print("\nFinal Model with LogisticRegressionCV:")   
logistic_regression2 = LogisticRegression(penalty='l1', solver='liblinear', C=logistic_cv.C_[0])
logistic_regression2.fit(X_train, y_train)
y_pred1 = logistic_regression2.predict(X_test)
...
non_zero_coefficients = (logistic_regression2.coef_ != 0).sum()
total_coefficients = logistic_regression2.coef_.size
print(f"Non-zero coefficients: {non_zero_coefficients}/{total_coefficients}")

# Feature importance
coefficients = logistic_regression2.coef_[0]
